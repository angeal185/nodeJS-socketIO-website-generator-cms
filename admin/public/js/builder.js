...
$("#detailTabs > li").first().addClass("active");
$('#toAdmin').removeClass('actionButtons disabled');
...
var editableContent = data.editableContent;
  ...
  for (x = 0; x <= 99; x++) {
    ...
    if (localStorage.getItem("blocksElement" + x) !== null && localStorage.getItem("blocksFrame" + x) !== null) {
      ...
      if (x >= 1 && pageNames[x] != undefined) {
        ...
        newLI = $('<li><a class="plink" href="#' + pageNames[x] + '">' + pageNames[x] + '</a><span class="pageButtons"><a class="fileEdit" href=""><span class="fa fa-clone"></span></a><a class="fileDel" href=""><span class="fa fa-times"></span></a><a href="#" class="btn btn-xs fileSave"></a></span></li>');
  ...
  el.sortable({
    ...
    beforeStop: function(event, ui) {
      ...
      if (ui.item.find('.frameCover').length == 0) {
        ...
        //add a delete button
        delButton = $('<button type="button" class="btn btn-default deleteBlock">remove</button>');
        resetButton = $('<button type="button" class="btn btn-default resetBlock">reset</button>');
        htmlButton = $('<button type="button" class="btn btn-default htmlBlock">source</button>');
...
function styleClick(el) {
  ...
  $('#pageList ul:visible li iframe').each(function() {
    ...
    for (var key in editableItems) {
      ...
      $(this).contents().find(pageContainer + ' ' + key).css({
        'outline': 'none',
        'cursor': 'default'
      });
  ...
  if ($('#styleEditor').css('left') == '0px') {
    ...
    $('#pageList ul li iframe').each(function() {
      ...
      for (var key in editableItems) {
        ...
        $(this).contents().find(pageContainer + ' ' + key).unbind('mouseenter mouseleave click').css({
          'outline': '',
          'cursor': ''
        });
...
$(function() {
  ...
  for (var key in data.elements) {
    ...
    for (x = 0; x < data.elements[key].length; x++) {
      ...
      if (data.elements[key][x].thumbnail == null) { //we'll need an iframe

        //build us some iframes!

        if (data.elements[key][x].sandbox != null) {

          if (data.elements[key][x].loaderFunction != null) {
            loaderFunction = 'data-loaderfunction="' + data.elements[key][x].loaderFunction + '"';
          }

          newItem = $('<li class="element ' + niceKey + '"><iframe src="' + data.elements[key][x].url + '" scrolling="no" data-sandbox="" ' + loaderFunction + '></iframe></li>');
        ...
        newItem.find('iframe').attr('src', data.elements[key][x].url);
        ...
        if (data.elements[key][x].sandbox != null) {
          ...
          newItem = $('<li class="element ' + niceKey + '"><img src="' + data.elements[key][x].thumbnail + '" data-srcc="' + data.elements[key][x].url + '" data-height="' + data.elements[key][x].height + '" data-sandbox="" ' + loaderFunction + '></li>')

        } else {

          newItem = $('<li class="element ' + niceKey + '"><img src="' + data.elements[key][x].thumbnail + '" data-srcc="' + data.elements[key][x].url + '" data-height="' + data.elements[key][x].height + '"></li>')
  ...
  $('input:radio[name=mode]').on('toggle', function() {
    //$("label:has('.active')").removeClass('active');
        ...
        for (i = 0; i < editableContent.length; ++i) {
          ...
          $(this).contents().find(pageContainer + ' ' + editableContent[i]).hover(function() {
            ...
            if (this.tagName == 'SMALL' || this.tagName == 'A' || this.tagName == 'LI' || this.tagName == 'SPAN' || this.tagName == 'B' || this.tagName == 'I' || this.tagName == 'TT' || this.tageName == 'CODE' || this.tagName == 'EM' || this.tagName == 'STRONG' || this.tagName == 'SUB' || this.tagName == 'BUTTON' || this.tagName == 'LABEL' || this.tagName == 'P' || this.tagName == 'H1' || this.tagName == 'H2' || this.tagName == 'H2' || this.tagName == 'H3' || this.tagName == 'H4' || this.tagName == 'H5' || this.tagName == 'H6') {
              ...
              ClassicEditor
                .create( document.querySelector( '#editContentModal #contentToEdit' ), {
  ...
  $('button#updateContentInFrameSubmit').click(function() {
    ...
    elToUpdate.html(myEditor.getData().replace("<p>","").replace("</p>",""));
    ...
    if (sandboxID) {
      ...
      $('#' + sandboxID).contents().find('#' + elementID).html(myEditor.getData().replace("<p>","").replace("</p>",""));
    }
  ...
  $('#pageList').on("click", ".frameCover > .htmlBlock", function() {
    ...
    editor.setTheme("ace/theme/"+data.options.aceTheme);
  ...
  function createBtn(i){
    ...
    //buttons
    var btns = ["Options","Keys","Beautify","Minify","Cancel","Save"]

    buttonWrapper = $('<div class="editorButtons"></div>');
    btns.forEach(function(i) {
      buttonWrapper.append($('<button type="button" class="btn btn-default edit'+i+'Button btn-wide">'+i+'</button>'));
    });
    ...
    $(".editBeautifyButton").click(function() {
      let data = editor.getValue();
      socket.emit('beautifyBuilder', data);
      return false;
    });

    $(".editMinifyButton").click(function() {
      let data = editor.getValue();
      socket.emit('minifyBuilder', data);
      return false;
    });
    ...
    $('.editKeysButton').click(function() {
      editor.execCommand("showKeyboardShortcuts")
    });

    socket.addEventListener("beautifyBuilder", function (i) {
      editor.setValue(i.data);
    });

    socket.addEventListener("minifyBuilder", function (i) {
      editor.setValue(i.data);
    });
  ...
  $('#pages').on('click', 'li.active .fileDel', function() {
    ...
    $("button#deletePageConfirm").prev().attr('id', 'deletePageCancel');
  ...
  //content modal, destroy ckeditor when modal closes
  $('#editContentModal').on('hidden.bs.modal', function(e) {
    //console.log(myEditor.getData().replace("<p>","").replace("</p>",""));
    myEditor.destroy();
  })
